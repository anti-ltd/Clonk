...
//
// Fires only when the user taps "Check for updates" in the About tab — there
// is no background polling, no telemetry, no identifiers in the request. The
// endpoint (anti-ltd's release index) returns the metadata for the latest
// public build; we compare its `version` against the running bundle and
...
public enum UpdateChecker {
    ...
    public static func fetch(appID: String = "clonk",
...
// base so a server response of "/api/download?app=clonk" still opens to a
// real https:// URL.
public extension VersionInfo {
    func resolvedDownloadURL(relativeTo base: URL = UpdateChecker.endpoint) -> URL? {
        ...
        // host root, not at "/api1/version/api/download".
